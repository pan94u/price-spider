import { priceDB, originDB } from './model/price'
...
(async () => {
  await priceDB.sync(force)
  await modelDB.sync(force)
  await groupDB.sync(force)
  await originDB.sync(force)
})()


function start() {
  console.log(`开始写入数据库 in ${moment().format('YYYY-MM-DD HH:mm:ss')}`)
  loopData().then(async (result) => {
    //遍历全部分组
        ...
        detail.forEach((model, index) => {
          ...
          priceArr.forEach((value, index) => {
            ...
            modelDB.findAll({ where: { originText, country, color, groupId } }).then((result) => {
              if (isEmptyArr(result)) {
                //如果没有这个型号则新建
                modelDB.create({ originText, country, color, name, chartsUrl, groupId, weight_apt: modelIndex, status: 0 }).then((result) => {
                  modelId = result.id
